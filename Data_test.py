...
skd = 0
skkd = 0
for product in sales_collection.find({'Sold': {"$ne": 'N/A'}}):
    skkd += 1
skd = count - skkd

print(f'Số lượng dược phẩm trong db: {count}')
print(f'Số lượng dược phẩm kê đơn trong db: {skd}')
print(f'Số lượng dược phẩm không kê đơn trong db: {skkd}')

#Sắp xếp các dược phẩm theo số lượng bán giảm dần, loại bỏ các thuốc thuộc loại kê đơn
ckd = 0
for product in sales_collection.find({'Sold': {"$ne": 'N/A'}}).sort({'Sold': -1}):
    print(product)
